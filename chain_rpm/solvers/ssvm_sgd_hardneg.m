function [model, progress] = ssvm_sgd_hardneg(pos, neg, labels, model, params, options)
%

% parse the options
n = length(pos); % number of training examples
...
for p = 1:options.num_passes
    ...
    for dummy = 1:n
        ...
        % 2) 
        ystar_i = hardNegMine(params, model, pos{i}, labels{i});
        ...
        phi_gt = phi(params, model, pos{i}, labels{i});
        phi_y  = phi(params, model, pos{i}, ystar_i);
        psi_i = phi_gt - phi_y;
        ...
        if (options.debug && k == debug_iter)
            ...
            primal = primal_objective(params, maxOracle, pos, labels, model_debug, lambda);
            train_error = average_loss(params, maxOracle, pos, labels, model_debug);
