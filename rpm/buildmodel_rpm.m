function jointmodel = buildmodel_rpm(name,model,def,idx,K,pa,ov,ovu)
...
jointmodel.ominode = struct('w',{},'i',{});
jointmodel.omiedge = struct('w',{},'i',{});
jointmodel.components{1} = struct('biasid',{},'filterid',{},'defid',{},...
  'onid',{},'omid',{},'parent',{});
...
jointmodel.adj = zeros(length(pa));
for k = 2:length(pa)
  jointmodel.adj(k,jointmodel.pa(k)) = 1;
end
...
for i = 1:length(pa)
  ...
  % add omission
  p.onid = [];
  nb = length(jointmodel.ominode);
  b.w = ov(i);
  b.i = jointmodel.len + 1;
  jointmodel.ominode(nb+1) = b;
  jointmodel.len = jointmodel.len + numel(b.w);
  p.onid = nb+1;
  
  p.omid = [];
  if i > 1
    nb = length(jointmodel.omiedge);
    bb.w = ovu(i);
    bb.i = jointmodel.len + 1;
    jointmodel.omiedge(nb+1) = bb;
    jointmodel.len = jointmodel.len + numel(bb.w);
    p.omid = nb+1;
  end
